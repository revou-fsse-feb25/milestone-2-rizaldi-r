...
(window as any).onGameAnchorClicked = (gameTitle: string) => {
    const gameData = gameDataList.find((data) => gameTitle === data.gameTitle);
    renderPage(gamePage(gameData));
};

(window as any).onHomeAnchorClicked = () => {
    renderPage(homePage());
};

const addNavbarGameList = (gameDataList: Array<GameDataList>) => {
    let gameListString: string = "";
